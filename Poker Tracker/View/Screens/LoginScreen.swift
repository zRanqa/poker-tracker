...
struct LoginScreen: View {
    ...
    func login() {
        ...
        postRequest(to: API.login.url, jsonBody: jsonBody, token: nil) { (result: Result<APIResponse<String>, Error>) in
            ...
            case .success(let response):
                ...
                if response.status == "success" {
                    if response.data != nil {
                        ...
                        DispatchQueue.main.async {
                            appState.login(with: response.data!)
                        }
    ...
    func signup() {
        ...
        postRequest(to: API.createVerificationCode.url, jsonBody: jsonBody, token: nil) { (result: Result<APIResponse<EmptyResponse>, Error>) in
            DispatchQueue.main.async {
                isLoading = false
            }
            switch result {
            case .success(let response):
                print(response.status)
                if response.status == "success" {
                    savedUserInfo = UserAccountInfo(email: email, password: password, name: name)
                    verifictionCode = true
    ...
    func confirm_code() {
        ...
        let jsonBody = ["uuid": savedUserInfo!.uuid.uuidString, "code": filteredCode, "email": savedUserInfo!.email, "password": savedUserInfo!.password, "name": savedUserInfo!.name]
        
        postRequest(to: API.createAccount.url, jsonBody: jsonBody, token: nil) { (result: Result<APIResponse<EmptyResponse>, Error>) in
            DispatchQueue.main.async {
                isLoading = false
            }
            switch result {
